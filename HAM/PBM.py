...
def make_C_matrix(C_room, C_wall):
    C1, C2 = np.diag(C_room/20), np.diag(C_wall/20)
    return C1, C2
...
class thermoPBM():
    ...
    def calculate_T_dot(self, T_room, T_wall, u_room, u_wall, corrective_source_term=np.zeros(13)):
        rhs_room, rhs_wall = self.calculate_rhs(T_room, T_wall, u_room, u_wall)
        T_dot_room = np.linalg.inv(self.C1) @ (rhs_room + corrective_source_term)
    ...
    def step(self, T_room, T_wall, u_room, u_wall, delta_t, corrective_source_term=np.zeros(13)):
