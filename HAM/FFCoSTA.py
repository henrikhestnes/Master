...
class FFCoSTA(nn.Module):
    ...
    def forward(self, T_room, T_wall, T_out, door, timing, N, delta_t):
    ...
    def train(self, train_loader, val_loader, epochs, lr, l2_reg):
        ...
        for epoch in range(epochs):
            train_mae = 0
            for i, (temp, label, warmup_temp, warmup_outdoor, outdoor, door, timing) in enumerate(train_loader):
                optimizer.zero_grad()
                ...
                T_room_new, T_wall_new = self(T_room, T_wall, outdoor, door, timing, N, delta_t)
